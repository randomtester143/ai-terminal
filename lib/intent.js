// Heuristic intent detection. Runs before any LLM call.
// Returns one of: "code", "command", "explanation", "general".
//
// For "code" intent, also detects whether a programming language is named.
// If not, the handler returns a clarifier without spending an LLM call.
...
export function detectIntent(prompt) {
    ...
    // Command intent: short imperative shell-tool questions
    if (COMMAND_DIRECT.test(text) || COMMAND_HINTS.test(text)) {
        return { kind: "command", language: null };
    }

    // Code intent: verb + code noun, OR named algorithm, OR explicit "code" keyword
    const looksLikeCode =
...
export function systemPromptFor(intent) {
    const base =
        ...
        "Never use markdown: no asterisks, no backticks, no code fences, no headers, no bullet symbols. " +
