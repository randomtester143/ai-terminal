...
/**
 * Extract the client IP from the request, preferring Vercel-specific headers.
 * Falls through: x-vercel-forwarded-for → x-forwarded-for → x-real-ip → socket.
...
export async function rateLimit(ip) {
    if (!REDIS_OK || !redis || !ip) return { allowed: true, remaining: MAX_REQUESTS, retryAfter: 0 };
