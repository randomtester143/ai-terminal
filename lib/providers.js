...
const MAX_TOKENS = 4096; // FIXED: Increased limit to prevent mid-code cutoffs
...
export async function callHF(messages, systemPrompt, isRetry = false) {
    ...
    try {
        ...
        if (!res.ok) {
            // FIXED: Added an automatic 2s retry if the model is waking up to make failover usable
